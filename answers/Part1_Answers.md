...
- **Short reviews (≤ q25 = 11 words):** 10,463 reviews
  - Average word count: **6.39 words**
- **Long reviews (≥ q75 = 179 words):** 10,034 reviews
  - Average word count: **493.00 words**
...
- **TF-IDF matrix shape:** (20,497 samples × 25,085 features)
  - Vocabulary size: 25,085 terms
  - Sparsity: 99.68% (only 0.32% of entries are non-zero)
- **MiniLM embedding matrix shape:** (20,497 samples × 384 dimensions)
  - All dimensions are populated (dense)
  - L2 normalized

**Why TF-IDF is sparse while MiniLM is dense:**

TF-IDF is a bag-of-words representation that assigns a score to each term in the vocabulary. Since:

1. Each review typically contains only a tiny fraction of the total vocabulary (e.g., a review of 50 words might use only 30 unique terms out of 25,085)
2. Unused terms get a score of exactly 0
3. The resulting matrix has many zero values → highly sparse

MiniLM embeddings are generated by a neural network that projects the entire sentence into a fixed 384-dimensional latent space. The model:

1. Processes the semantic meaning of the entire text, not individual word counts
2. Outputs a continuous vector where every dimension contributes to the representation
3. All 384 dimensions are activated with non-zero values → dense representation
...
**Best-performing pipeline overall:** **MiniLM: SVD(50) + Agglomerative Clustering**
...
**MiniLM significantly outperforms TF-IDF** for separating Short vs Long reviews based on all clustering metrics.
...
1. **Semantic understanding:** MiniLM captures semantic meaning and sentence structure. Long reviews inherently differ from short reviews in semantic complexity, topic depth, and discourse structure—differences that neural embeddings naturally encode.

2. **Document length as a latent signal:** Transformers attend to longer sequences differently, creating distinct internal representations for different-length inputs. The model implicitly learns that "tokens vary" and "sentence complexity varies" with length.

3. **TF-IDF bag-of-words limitations:** TF-IDF only counts word frequencies, ignoring:
   - Word order and grammar
   - Semantic relationships between words
   - Sentence structure

   Long reviews and short reviews can share many common words (e.g., "game", "play", "good") making them appear similar in TF-IDF space even when structurally different.

**Why UMAP fails for this task (for both TF-IDF and MiniLM):**

- UMAP preserves local manifold structure, but Short vs Long is a **global, linear signal** (document length affects all dimensions similarly)
- UMAP's focus on local neighborhoods may blur this global pattern
- SVD (linear) is more appropriate for capturing this simple length-based separation
...
**PCA reduction:** 2 components applied to both representations

- TF-IDF explained variance: 2.39% (PC1) + 1.63% (PC2) = **4.02% total**
- MiniLM explained variance: 8.71% (PC1) + 3.13% (PC2) = **11.83% total**

**Visualization components (2×2 grid):**

| Position     | Plot                    | Description                                                    |
| ------------ | ----------------------- | -------------------------------------------------------------- |
| Top-left     | TF-IDF: Ground Truth    | Colored by Short/Long labels - shows weak separation           |
| Top-right    | TF-IDF: Best Clustering | SVD + Agglomerative (V=0.047) - shows poor cluster quality     |
| Bottom-left  | MiniLM: Ground Truth    | Colored by Short/Long labels - shows moderate separation       |
| Bottom-right | MiniLM: Best Clustering | SVD + Agglomerative (V=0.648) - shows clear cluster separation |

**Key observations:**

- MiniLM points show more structured separation by length
- TF-IDF points appear more scattered with weak length-based structure
- The 2D PCA projection captures MiniLM structure better (11.83% vs 4.02% variance explained)
- Cluster labels in MiniLM visualization align well with ground truth colors
...
- **TF-IDF game matrix shape:** (200 games × 16,554 features)
  - Vocabulary derived from concatenating positive reviews per game
- **MiniLM game embedding matrix shape:** (200 games × 384 dimensions)
  - Each game vector = average of all positive review MiniLM embeddings
...
**Justification:** This pipeline achieved the best results in Task 1 (V=0.648, ARI=0.742) and shows strong genre purity in Task 2 (up to 100% for small clusters, 85% for larger ones). MiniLM captures semantic similarity better than TF-IDF for game content, and SVD dimensionality reduction improves clustering efficiency while preserving semantic structure.
...
**Interpretation:** This cluster represents pure action-focused competitive multiplayer games. These are primarily online multiplayer titles with fast-paced combat mechanics. The 100% purity indicates all games in this cluster contain the Action genre label.
...
**Interpretation:** This cluster represents Action-Adventure-RPG hybrid games—narrative-driven action titles with RPG progression elements. This is the largest cluster of mainstream AAA titles, featuring games that blend combat, exploration, and character development systems.
...
**Evidence provided per cluster:**

- 10 top TF-IDF terms (unigrams and bigrams)
- 2 exemplar reviews (truncated to 500 characters if necessary)
...
| Question | Task | Topic                          |
| -------- | ---- | ------------------------------ |
| Q1       | 1.1  | Review count & average lengths |
| Q2       | 1.2  | TF-IDF vs MiniLM dimensions    |
| Q3       | 1.3  | Clustering metrics table       |
| Q4       | 1.4  | Representation comparison      |
| Q5       | 1.4  | Visualization plots            |
| Q6       | 2.1  | Game matrix dimensions         |
| Q7       | 2.2  | Pipeline cluster summaries     |
| Q8       | 2.3  | High-purity cluster analysis   |
| Q9       | 3.1  | Held-out game genre estimation |
| Q10      | 3.2  | Negative review themes         |
| Q11      | 3.2  | Positive review themes         |
| Q12      | 3.2  | LLM labeling strategy          |

**Key Findings:**

1. **MiniLM outperforms TF-IDF** for semantic clustering (13-148× better metrics in Task 1)
2. **SVD + Agglomerative** is the most reliable pipeline across all tasks
3. **Genre clustering works** - achieved up to 100% purity for small clusters
4. **Held-out game** assigned to Action-Adventure-RPG cluster (80% Action, 55% Adventure, 35% RPG)
5. **Negative themes**: Boss difficulty, technical issues, boring gameplay, poor level design
6. **Positive themes**: Visual excellence, fair difficulty, engaging combat, cultural depth
